...
function QAProjectListComponent() {
    ...
    return (
        <Container className="m-o p-3">
            <h5 className={ styles.heading }>Automation Projects</h5>
            <Row className="row-cols-auto text-start">
                {/* add cards from json file */
                    Array.from({ length: 3 }).map((_, id) => (
                        <Col key={`qa-${id}`} className="mx-auto">
                            <Card className={ styles.card }>
                                ...
                                <Link role="button" to={`/automation/uiux/${id+1}`} className={ styles.btn }>View Details</Link>
...
export default QAProjectListComponent;